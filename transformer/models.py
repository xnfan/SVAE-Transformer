...
class Encoder(nn.Module):
    def __init__(self, n_layers, d_k, d_v, d_model, d_ff, n_heads,
        ...
        self.src_emb = nn.Embedding(src_vocab_size, d_model, padding_idx=data_utils.PAD,).cuda()
        self.pos_emb = PosEncoding(max_seq_len * 10, d_model).cuda() # TODO: *10 fix
...
class Transformer(nn.Module):
    def __init__(self, opt):
        ...
        self.mu_linear = nn.Linear(opt.d_model, 1024)
        self.var_linear = nn.Linear(opt.d_model, 1024)
        self.vae_z_linear = nn.Linear(1024, opt.d_model)
    ...
    def decode(self, dec_inputs, dec_inputs_len, enc_inputs, enc_outputs, return_attn=False):
        mu = self.mu_linear(enc_outputs)
        var = self.var_linear(enc_outputs)

        z = torch.randn(enc_outputs.shape[0], enc_outputs.shape[1], 1024).cuda()
        z = self.vae_z_linear(z).cuda()
        new_enc_outputs = [enc_outputs+z, enc_outputs]
        return self.decoder(dec_inputs, dec_inputs_len, enc_inputs, new_enc_outputs, return_attn)
    ...
    def forward(self, enc_inputs, enc_inputs_len, dec_inputs, dec_inputs_len, return_attn=False):
        ...
        z = self.vae_z_linear(z)
        new_enc_outputs = [enc_outputs+z, enc_outputs]
        dec_outputs, dec_self_attns, dec_enc_attns = \
            self.decoder(dec_inputs, dec_inputs_len, enc_inputs, new_enc_outputs, return_attn)
        dec_logits = self.tgt_proj(dec_outputs)

        return dec_logits.view(-1, dec_logits.size(-1)), \
               enc_self_attns, dec_self_attns, dec_enc_attns, mu, var
