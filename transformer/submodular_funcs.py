...
embedding_path = os.path.join('./data', 'embeddings', 'quora_word2vec.pickle')
model = None
...
def similarity_func(v, S):
    if len(S):
        ...
        for sent in S:
            score+= sentence_compare(v[0], sent, kernel='cos', sigma=1.0)
...
def distinct_src_ngrams_func(v, S):
    #print(S)
    if len(S):
        S = ' '.join(S)
        N = [1,2,3]
        ...
        score = 0.0
        for n in N:
            src_tokens = len(ngram_toks(v, n))-1
            toks = set(ngram_toks([S], n))
            score+= (1.0/n) * (len(toks) - src_tokens)

        return np.sqrt(score)
    else:
        return 0.0

def distinct_src_ngrams_gain(v, s, S, base_score = 0.0):
    S = [' '.join(x) for x in S]
    #print(S)
    if len(S):
        S = ' '.join(S)
        ...
        gain = 0.0
        for n in N:
            src_tokens = len(ngram_toks(v, n))
            toks = set(ngram_toks([S], n))
            gain+= (1.0/n) * (len(toks) - src_tokens)

        return np.sqrt((base_score**2) + gain)
    else:
        return 0.0
    ...
    if len(S):
        ...
        for n in N:
            ...
            for sent in S:
                ...
                loverlap = min(len(overlap), 0.7*len(v[0].split()))
                score += (1.0/(4-n)) * loverlap
...
def ngram_overlap_unit(v, S , base_score=0.0):
    ...
    for n in N:
        ...
        loverlap = min(len(overlap), 0.7*len(src_toks))
...
def jkd_func(V, S):
    if len(S):
        ...
        for v in V:
            ...
            for s in S:
                s_set = set(s)
                and_set = s_set.intersection(v_set)
                or_set = s_set.union(v_set)
                score_temp = 1 - float(len(and_set)) / len(or_set)
                score += score_temp
...
def jkd_gain(V, s, base_score= 0.0):
    gain =0.0
    s_set = set(s)
    for v in V:
        v_set = set(V)
