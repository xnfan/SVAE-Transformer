...
class _MultiHeadAttention(nn.Module):
    def __init__(self, d_k, d_v, d_model, n_heads, dropout):
        ...
        self.w_q = Linear(d_model, d_k * n_heads, bias=False)
        self.w_k = Linear(d_model, d_k * n_heads, bias=False)
        self.w_v = Linear(d_model, d_v * n_heads, bias=False)
    ...
    def forward(self, q, k, v, attn_mask):
        ...
        # print("attn_masks", attn_mask.size())
        # print("attn_masks", attn_mask.unsqueeze(1).size())
        # print("attn_masks", attn_mask.unsqueeze(1).repeat(1, self.n_heads, 1, 1).size())
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask):
        ...
        residual = q
        # context: a tensor of shape [b_size x len_q x n_heads * d_v]
        context, attn = self.multihead_attn(q_norm, k_norm, v_norm, attn_mask=attn_mask)
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs):
        inter = self.dropout(self.relu(self.w1(self.layer_norm(inputs))))
        output = self.dropout(self.w2(inter))
