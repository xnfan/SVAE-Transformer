...
def train(model, criterion, optimizer, train_iter, model_state):  # TODO: fix opt
    ...
    kl_loss_now, kl_loss_total = 0.0, 0.0
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        #print("dec_inputs_len", type(dec_inputs_len), dec_inputs_len, dec_inputs_len.size())

        # Execute a single training step: forward
        optimizer.zero_grad()
        dec_logits, _, _, _, mu, var = model(enc_inputs, enc_inputs_len,
                                    dec_inputs, dec_inputs_len)
        step_loss = criterion(dec_logits, dec_targets.contiguous().view(-1))

        step = model_state['train_steps']
        kl = torch.sum((var - var.exp() - mu.pow(2) + 1)* -0.5)/(var.shape[0] * 2)
        kl_weight = float(1/(1 + np.exp(-0.0025*(step-2500))))
        kl_loss = kl_weight * kl

        # Execute a single training step: backward
        loss = step_loss + kl_loss
        loss.backward()
        ...
        #print("step_loss_size", step_loss.size())
        train_loss_total += float(step_loss.item())
        kl_loss_total += float(kl_loss.item())
        n_words_total += int(torch.sum(dec_inputs_len).item())
        n_sents_total += int(dec_inputs_len.size(0))  # batch_size
        model_state['train_steps'] += 1
        #print(train_loss_total, n_words_total, n_sents_total, n_words, n_sents)

        # Display training status
        if model_state['train_steps'] % opt.display_freq == 0:
            kl_loss_int = (kl_loss_total - kl_loss_now)
            ...
            print('Epoch {0:<3}'.format(model_state['curr_epochs']), 'Step {0:<10}'.format(model_state['train_steps']),
                  ...
                  '{0:.2f} sents/s'.format(n_sents_sec), '{0:>10.2f} words/s'.format(n_words_sec),'{0:>10.2f} kl_loss/steps'.format(kl_loss_int))
            train_loss, n_words, n_sents = (train_loss_total, n_words_total, n_sents_total)
            kl_loss_now = kl_loss_total 
            start_time = time.time()
